...
from synflownet.algo.config import Backward, LossFN, NLoss, TBVariant
from synflownet.algo.graph_sampling import Sampler
from synflownet.config import Config
from synflownet.envs.graph_building_env import (
    ActionIndex,
    Graph,
    GraphAction,
...
from synflownet.trainer import GFNAlgorithm
...
class TrajectoryBalance(GFNAlgorithm):
    ...
    def __init__(
        ...
        assert (
            self.cfg.backward_policy
            not in [Backward.MaxLikelihood, Backward.REINFORCE, Backward.Free]
            or self.cfg.do_parameterize_p_b
        ), "can't do MaxlikeLihood, REINFORCE or Free without parametrising P_B"
    ...
    def create_training_data_from_own_samples(
        ...
        strict_forward_policy: Optional[GraphActionCategorical] = None,
    ):
        ...
        data = self.sampler.sample_from_model(
            model, n, cond_info, random_action_prob, strict_forward_policy=None
        )
    ...
    def get_idempotent_actions(
        ...
        """
        iaction = self.ctx.GraphAction_to_ActionIndex(
            gd, action, fwd=not action.action.is_backward
        )
        ...
        for i in nz:
            ...
            if ga.action == GraphActionType.ReactBi:
                # go through all possible bbs
                bb_masks = torch.tensor(
                    self.ctx.create_masks_for_bb_from_precomputed(g, aidx.col_idx)
                )
                nz_bb = bb_masks.nonzero()
                for j in nz_bb:
                    aidx = ActionIndex(
                        action_type=iaction[0], row_idx=i[1].item(), col_idx=j[0].item()
                    )
                    ga = self.ctx.ActionIndex_to_GraphAction(
                        gd, aidx, fwd=not action.action.is_backward
                    )
                    child = self.ctx.obj_to_graph(self.env.step(g, ga))
    ...
    def construct_batch(self, trajs, cond_info, log_rewards):
        ...
        if self.model_is_autoregressive:
            torch_graphs = [
                self.ctx.graph_to_Data(tj["traj"][-1][0], traj_len=k)
                for tj in trajs
                for k, _ in enumerate(tj["traj"])
            ]
            actions = [
                self.ctx.GraphAction_to_ActionIndex(g, i[1], fwd=True)
        ...
        batch.ends_in_s_0 = torch.tensor(
            [i.get("ends_in_s_0", True) for i in trajs]
        ).float()
        batch.from_p_b = torch.tensor([i.get("from_p_b", True) for i in trajs]).float()
        batch.bck_rewards = torch.tensor(
            [i.get("bck_reward", 1) for i in trajs]
        ).float()
    ...
    def compute_batch_losses(
        ...
        invalid_bck_mask = 1 - batch.ends_in_s_0
        ...
        if self.cfg.do_parameterize_p_b:
            ...
            log_p_B[batch.is_sink == 1] = 0.0
            for i, l in enumerate(log_p_B):
                if l <= -900:
                    raise ValueError(
                        f"Invalid log_p_B: {l}, {i}, {batch.bck_actions[i+1]}, {Chem.MolToSmiles(self.ctx.graph_to_obj(batch.nx_graphs[i+1]))}"
                    )
        else:
        ...
        if self.cfg.variant == TBVariant.SubTB1:
            ...
            if self.cfg.backward_policy in [Backward.MaxLikelihood, Backward.REINFORCE]:
                numerator = (
                    log_Z[batch.from_p_b == 0] + traj_log_p_F[batch.from_p_b == 0]
                )
                denominator = (
                    clip_log_R[batch.from_p_b == 0]
                    + traj_log_p_B[batch.from_p_b == 0].detach()
                )
            else:
        ...
        if self.cfg.backward_policy == Backward.REINFORCE:
            # If we use REINFORCE for bck policy, we need to mask out traj_log_p_B that don't come from sampling from P_B
            # However, using trajectories from p_f helps with stability
            invalid_bck_mask = invalid_bck_mask[batch.from_p_b == 1]
            # traj_log_p_B = traj_log_p_B[batch.from_p_b == 1]
            # bck_rewards = batch.bck_rewards[batch.from_p_b == 1]
            pB_reinforce_loss = -(traj_log_p_B * batch.bck_rewards).mean() + self.cfg.bck_entropy_loss_multiplier * (
                -bck_cat.entropy().mean()
            )
        else:
            pB_reinforce_loss = 0
        if self.cfg.backward_policy == Backward.MaxLikelihood:
            pB_mle_loss = -traj_log_p_B.mean()
        else:
            pB_mle_loss = 0
        n_loss = n_loss.mean()
        ...
        loss = (
            ...
            + self.cfg.mle_loss_multiplier * pB_mle_loss
            + self.cfg.reinforce_loss_multiplier * pB_reinforce_loss
        )
        info = {
            "offline_loss": (
            ...
            "online_loss": (
                traj_losses[batch.num_offline :].mean() if batch.num_online > 0 else 0
            ),
            "online_loss": tb_loss,
            "reward_loss": reward_loss,
            "bck_reward": batch.bck_rewards[batch.from_p_b == 1].mean(),
            "invalid_trajectories": (
                invalid_mask[batch.from_p_b == 0].sum() / batch.num_online
                if batch.num_online > 0
                else 0
            ),
            "invalid_bck_trajectories": (
                invalid_bck_mask.sum() / batch.from_p_b.sum()
                if Backward.REINFORCE
                else 0
            ),
            "invalid_logprob": (
                invalid_mask[batch.from_p_b == 0] * traj_log_p_F[batch.from_p_b == 0]
            ).sum()
            / (invalid_mask[batch.from_p_b == 0].sum() + 1e-4),
            "logZ": log_Z.mean(),
            "loss": loss.item(),
            "n_loss": n_loss,
            "pB_mle_loss": pB_mle_loss,
            "pB_reinforce_loss": pB_reinforce_loss,
            "tb_loss": tb_loss.item(),
            "log_p_F": traj_log_p_F.mean(),
            "log_p_B_from_p_b": traj_log_p_B[batch.from_p_b == 1].mean(),
            "log_p_B_from_p_f": traj_log_p_B[batch.from_p_b == 0].mean(),
